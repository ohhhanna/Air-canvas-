...
# Initialize deque arrays for storing points of different colors
bpoints = [deque(maxlen=1024)]
gpoints = [deque(maxlen=1024)]
rpoints = [deque(maxlen=1024)]
ypoints = [deque(maxlen=1024)]

# Indexes for each deque array
blue_index = 0
green_index = 0
red_index = 0
yellow_index = 0
...
while True:
    ...
    # Create the mask for detecting the pointer
    Mask = cv2.inRange(hsv, Lower_hsv, Upper_hsv)
    Mask = cv2.erode(Mask, kernel, iterations=1)
    Mask = cv2.morphologyEx(Mask, cv2.MORPH_OPEN, kernel)
    Mask = cv2.dilate(Mask, kernel, iterations=1)
    
    # Find contours of the detected pointer
    ...
    if len(cnts) > 0:
        ...
        # Check if the pointer interacts with the buttons
        if center[1] <= 65:
            ...
            # Add the detected points to the respective deque
            if colorIndex == 0:
        ...
        # Add new deques for drawing when the pointer is not detected
